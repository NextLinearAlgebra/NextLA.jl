...
int main(int argc, char** argv) {
    ...
    // ── BC 2.5D + slab Full-2.5D dispatch (all 4 matrix modes) ──────────────
    // BC 2.5D is the default; --layout=slab requests the legacy slab path.
    {
        ...
        if (use_full25d) {
            ...
            int rc;
            if (A.bc25d_layout) {
                if (_rank == 0) print_full25d_grid(G, "qdwh_bc25d", A.M_fp64_words, A.b);
                {
                    int b_snapped = snap_b_to_divisor(A.N, A.b);
                    if (b_snapped != A.b) {
                        if (_rank == 0) fprintf(stdout,
                            "qdwh_bc25d: snapping b %d -> %d (divisor of N=%d in §A3b window)\n",
                            A.b, b_snapped, A.N);
                        A.b = b_snapped;
                    }
                }
                if (A.N % A.b != 0) {
                    if (_rank == 0) fprintf(stderr, "qdwh_bc25d: N=%d must be divisible by b=%d\n", A.N, A.b);
                    MPI_Abort(MPI_COMM_WORLD, 65);
                }
                if (A.matrix == MatrixMode::FP32_FULL) {
                    rc = run_qdwh_bc25d_fp64(A.N, A.b, A.iters, A.lookahead,
                                              /*use_mp_trail=*/true,
