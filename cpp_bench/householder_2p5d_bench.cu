...
int main(int argc, char** argv) {
    ...
    // Activates when the user (or auto-derived schedule) selects Pz>1.  Only
    // wired for FP64; the MP / TF32 / fp32full paths fall through to the
    // legacy 1D path below.  See tex §sec:per-variant-schedule.
    {
        ...
        if (use_full25d && A.matrix == MatrixMode::FP64) {
            ...
            if (_rank == 0) print_full25d_grid(G, "householder_full25d", A.M_fp64_words, A.b);
            int rc = run_householder_full25d_fp64(A, G, S);
