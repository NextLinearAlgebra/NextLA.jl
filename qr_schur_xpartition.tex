...
The full-grid version is implemented in
\texttt{scqr3\_full25d\_bench.cu} (Path-s, sCQR3 / CQR2 with passes
$\in\{2,3\}$, look-ahead default-on with \texttt{--no-la} ablation) and
shares its $P_x{\times}P_y{\times}P_z$ scaffolding via the
\texttt{full25d\_grid.hpp} header.  Each of the single-path benches
(\texttt{householder\_2p5d\_bench.cu}, \texttt{givens\_2p5d\_bench.cu},
\texttt{qdwh\_2p5d\_bench.cu}) accepts \texttt{--px --py --pz} flags
that select the same Conflux $[P_x, P_y, P_z]$ grid; when the user
omits them, the grid defaults to whatever
...
hand-tuning.  All five paths $\{$sCQR3, CQR2, Householder, Givens
(tournament panel), QDWH$\}$ also share the four matrix-mode flags
\texttt{--matrix=fp64|fp64mp|fp64mp\_tf32|fp32full} and the
\texttt{--la}/\texttt{--no-la} Phase Q5 toggle, giving the 40-combo
sweep per $N$ described in \texttt{run\_full25d\_oom\_sweep.sh}.
