...
\begin{tabular}{lll}
\toprule
                            & Path~(s) sCQR3 & Path~(h) Householder--WY \\
\midrule
Panel DAAP class            & DAAP-proper          & Quasi-DAAP \\
Panel constant (3 sCQR3 iters)& $3$                & $\log_2 P_r$ \\
Panel flops                 & $\sim 3 m b^2$       & $\sim m b^2$ \\
Trailing-update flops       & $\sim 2 N^2 m$ (same)& $\sim 2 N^2 m$ (same) \\
Trailing-update DAAP class  & DAAP-proper          & DAAP-proper \\
Conditioning tolerance      & $\kappa \le \mathbf{u}^{-1}$ & any $\kappa$ \\
2.5D replication factor     & $c = PM/N^2$ (\S\ref{sec:A3}) & $c = PM/N^2$ (\S\ref{sec:A3}) \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Phase Q5 --- Look-Ahead Pipelining (scheduling, both paths)}
...
With $s$ look-ahead streams, the schedule decomposes the trailing
update into $s$ column slabs of width $b$, $b$, $\dots$, $N{-}sb$.
Panel-$(k{+}1), \dots, $ Panel-$(k{+}s)$ can be pre-factorized in
parallel \emph{after} their respective slabs are updated. This
overlaps up to $s$ panels' worth of work behind the trailing-rest GEMM.
The X-partition admissibility per stream requires
$M_{\text{stream}} = M/s$ words of fast memory, so the cube side
$\sqrt{M_{\text{stream}}} = \sqrt{M}/\sqrt{s}$ shrinks --- the
panel-per-stream bandwidth grows by $\sqrt{s}$. At $s = \sqrt{P_1}$
this exactly cancels the look-ahead savings. The X-partition optimum
is therefore $s \in [2, \sqrt{P_1}]$, with $s = 2$ being the standard
choice that preserves the cube side.
