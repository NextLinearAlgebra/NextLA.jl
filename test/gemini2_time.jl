...
# Include the file containing your latest FullMixedPrec, TriMixedPrec, and getrf_recursive! definitions
# include("lu_fullmixed_trimixed.jl") # <-- Ensure this filename matches your implementation file
...
function run_time_mixed_lu(A_fp32::CuMatrix{Float32}, precisions::Vector{DataType})
    ...
    # FP16 quantization, scaling, and structure allocation are excluded from LU kernel timing
    reset_op = () -> begin
        A_mixed_input = FullMixedPrec(copy(A_fp32); precisions=precisions)
        KernelAbstractions.synchronize(backend)
    end
    
    # Hits getrf_recursive!(A::FullMixedPrec) which dynamically wraps views in TriMixedPrec
...
function check_lu_time_hierarchical()
    ...
    println("="^60)
    println("Starting TriMixedPrec getrf_recursive! Benchmark (2k - 16k)")
    println("="^60)
    ...
    # ==========================================================================
    plt = plot(
        title="TriMixedPrec LU Performance vs. Matrix Size",
    ...
    savefig(plt, "trimixedprec_lu_runtimes.png")
    println("\nPlot saved as trimixedprec_lu_runtimes.png")
end
