...
@testset "native blocked potrf" begin
...
@testset "native batched blocked potrf" begin
    batch_count = 3
...
@testset "native blocked potrf split outer tails" begin
    batch_count = 3
...
@testset "native blocked potrf failure status" begin
...
@testset "wrapper potrf_batched!" begin
    batch_count = 3
        ...
        for n in (48, 50)
            ...
            for uplo in ('L', 'U')
                ...
                for (name, AT, sync) in backends
                    @testset "$name $T n=$n uplo=$uplo" begin
                        A_dev = _potrf_to_backend(AT, A_host)
                        @test occursin(_expected_potrf_batched_file(name), _method_file(NextLA.potrf_batched!, uplo, A_dev))
                        if name == "Metal"
                            @test_throws ArgumentError NextLA.potrf_batched!(uplo, A_dev)
                            continue
                        elseif name == "oneAPI"
                            NextLA.potrf_batched!(uplo, A_dev)
                        else
                            _, status_dev = NextLA.potrf_batched!(uplo, A_dev)
                            @test Array(status_dev) == zeros(Int32, batch_count)
                        end
                        sync(A_dev)
...
@testset "wrapper potrf_batched! failure status" begin
    n = 32
...
@testset "wrapper potrf_batched! tail failure status" begin
    n = 49
