...
@testset "Cholesky Accuracy Check" begin
    ...
    tolerance = 1e-5 # slightly looser for Float32 accumulation differences
...
function run_chol_benchmark()
    ...
    for n in n_sizes
        ...
        d_A_init = CuArray(A_host) # Clean copy source
        d_A_work = CuArray(A_host) # Workspace
        
        backend = KernelAbstractions.get_backend(d_A_work)

        # 1. Benchmark Right-Looking
        op_right = () -> cholesky_right!(d_A_work)
        reset_func = () -> copyto!(d_A_work, d_A_init)
        t_right = benchmark_op(op_right, reset_func, backend) / 1e6

        # 2. Benchmark Left-Looking
        op_left = () -> cholesky_left!(d_A_work)
        t_left = benchmark_op(op_left, reset_func, backend) / 1e6

        # 3. Benchmark CUSOLVER
        # Note: we need a fresh array because CUSOLVER is in-place
        ...
        speedup_ref_vs_left = t_left / t_cusolver # How much faster is CUSOLVER than Left?
