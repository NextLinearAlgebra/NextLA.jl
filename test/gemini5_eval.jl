...
# --- Streamlined Implementation Includes ---
# Uncomment these when running in your local workspace:
...
"""
function reconstruct_and_rescale(A::FullMixedPrec{T_Base}) where {T_Base}
    n, m = size(A)
    ...
    A11 = reconstruct_and_rescale(A.A11)
    A22 = reconstruct_and_rescale(A.A22)

    # Safely extract off-diagonal views (handling property name flexibility)
    off12_val = hasproperty(A, :A12) ? A.A12 : (hasproperty(A, :OffDiag) ? A.OffDiag : nothing)
    off21_val = hasproperty(A, :A21) ? A.A21 : (hasproperty(A, :OffDiag) ? A.OffDiag : nothing)

    A12 = T_Base.(Array(off12_val))
    A21 = T_Base.(Array(off21_val))

    if hasproperty(A, :A12_scale) && A.A12_scale !== nothing
        A12 .*= T_Base(A.A12_scale)
    end

    if hasproperty(A, :A21_scale) && A.A21_scale !== nothing
...
function get_accuracy_pure_lu(A_fp64::CuMatrix, T_prec::DataType, block_size::Int=256)
    ...
    # Hit your flat recursive driver (explicit block_size prevents UndefVarError)
...
function get_accuracy_mixed_lu(A_fp64::CuMatrix, precisions::Vector)
    ...
    A_result = reconstruct_and_rescale(A_mixed_input)
