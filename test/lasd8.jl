...
@testset "lasd8! test random input" begin
    ...
    # for T in [Float64,]
        ...
        for i in 1:200 
            ...
            # println("$T vf: $(vf)")
            # println("$T vf_copy: $(vf_copy)")
            # println("$T vl: $(vl)")
            # println("$T vl_copy: $(vl_copy)")
            # println("$T difr: $(difr)")
            # println("$T difr_copy: $(difr_copy)")
            # println("$T difl: $(difr)")
            # println("$T difl_copy: $(difl_copy)")
            # println()
            # println()
            ...
            if !isapprox(d, d_copy) && T == Float32
                ...
                # if !isapprox(d, d_copy_copy)
                #     println("About to fail secondary tests")
                # end
                ...
                # NextLA.lasd8!(icompq, k, d_copy_copy_copy, z_copy_copy_copy, vf_copy_copy_copy, 
                # vl_copy_copy_copy, difl_copy_copy_copy, difr_copy_copy_copy,
                # lddifr, dsigma_copy_copy_copy, work_copy_copy_copy, info)
                # @test isapprox(d_copy_copy_copy, d_copy_copy)
                # @test isapprox(z_copy_copy_copy, z_copy_copy)
                # @test isapprox(vf_copy_copy_copy, vf_copy_copy)
                # @test isapprox(vl_copy_copy_copy, vl_copy_copy)
                # @test isapprox(difl_copy_copy_copy, difl_copy_copy)
                # @test isapprox(difr_copy_copy_copy, difr_copy_copy)
                # @test isapprox(work_copy_copy_copy, work_copy_copy)
                # @test info[1] == info_copy[]
            else
