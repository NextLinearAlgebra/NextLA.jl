...
function reconstruct_matrix(A::SymmMixedPrec{T_Base}) where {T_Base}
    ...
    C_full = ROCArray{T_Recon}(undef, n, n)
...
# CHANGE: CuMatrix -> ROCMatrix
function get_accuracy_pure(A_spd_fp64::ROCMatrix, T_prec::DataType)
...
# CHANGE: CuMatrix -> ROCMatrix
function get_accuracy_mixed(A_spd_fp64::ROCMatrix, precisions::Vector)
...
# CHANGE: Renamed to generic "vendor" (uses rocSOLVER under hood)
# CHANGE: CuMatrix -> ROCMatrix
function get_accuracy_vendor(A_spd_fp64::ROCMatrix, T_prec::DataType)
    A_to_factor = T_prec.(A_spd_fp64)
    
    # CHANGE: CUSOLVER -> LinearAlgebra.cholesky!
    # AMDGPU's cholesky! uses rocSOLVER. We wrap in Hermitian to ensure correct dispatch.
    cholesky!(Hermitian(A_to_factor, :L))
...
function check_cholesky_accuracy()
    n_values = [1000, 4096, 5000, 8192, 9999, 16384, 20480, 24576, 28672, 32768, 65536] #256, 512, 1024, 2048, 
    ...
    # CHANGE: Renamed keys to Vendor/ROCSOLVER
    vendor_scenarios = Dict(
        "ROCSOLVER F32" => Float32,
        "ROCSOLVER F64" => Float64,
    )
    ...
    for n in n_values
        ...
        # CHANGE: CuArray -> ROCArray
        A_gpu_rand = ROCArray(A_cpu_rand)
        A_cpu_rand = nothing
        ...
        println("\n--- ROCSOLVER Library Scenarios ---")
        for (name, T_prec) in vendor_scenarios
            relative_error = get_accuracy_vendor(A_spd_fp64, T_prec)
