...
# include("matmul.jl")
# include("rectrxm.jl")
# include("recqr.jl") # <-- Ensure this matches where you saved the implementation above
...
function get_accuracy_cusolver_qr(A_fp64::CuMatrix, T_prec::DataType)
    ...
    return max(rel_err, 1e-20), max(orth_err, 1e-20), t_factor
end
...
function get_accuracy_mixed_qr(A_fp64::CuMatrix, precisions::Vector, block_size::Int=256)
    ...
    # (kept outside the timed region, mirroring the timing script's convention
    # of excluding quantization/allocation from factorization time)
...
function run_qr_accuracy_benchmark()
    n_values = [512, 1024, 2048, 4096, 8192] # Adjusted for QR which scales O(N^3)
    ...
    println("="^70)
    println("Starting QR Factorization (Elmroth-Gustavson) Accuracy Benchmark")
    println("(now with per-scenario wall-clock timing)")
    println("="^70)
    ...
    for n in n_values
        ...
        # Standard normal distribution is appropriate for testing QR stability
        ...
        for (name, T_prec) in cusolver_scenarios
            try
                rel_err, orth_err, t_factor = get_accuracy_cusolver_qr(A_fp64, T_prec)
                push!(all_rel_err[name], -log10(rel_err))
                push!(all_orth_err[name], -log10(orth_err))
                push!(all_time[name], t_factor)
                println("    $(rpad(name, 25)) | Rel. Error: $(rpad(round(rel_err, sigdigits=3), 8)) | Orth: $(rpad(round(orth_err, sigdigits=3), 8)) | Time: $(round(t_factor, digits=3)) s")
            catch e
                push!(all_rel_err[name], NaN)
                push!(all_orth_err[name], NaN)
                push!(all_time[name], NaN)
        ...
        for (name, T_prec) in pure_scenarios
            try
                rel_err, orth_err, t_factor = get_accuracy_pure_qr(A_fp64, T_prec)
                push!(all_rel_err[name], -log10(rel_err))
                push!(all_orth_err[name], -log10(orth_err))
                push!(all_time[name], t_factor)
                println("    $(rpad(name, 25)) | Rel. Error: $(rpad(round(rel_err, sigdigits=3), 8)) | Orth: $(rpad(round(orth_err, sigdigits=3), 8)) | Time: $(round(t_factor, digits=3)) s")
        ...
        for (name, prec_list) in mixed_scenarios
            try
                rel_err, orth_err, t_factor = get_accuracy_mixed_qr(A_fp64, prec_list)
                push!(all_rel_err[name], -log10(rel_err))
                push!(all_orth_err[name], -log10(orth_err))
                push!(all_time[name], t_factor)
                println("    $(rpad(name, 25)) | Rel. Error: $(rpad(round(rel_err, sigdigits=3), 8)) | Orth: $(rpad(round(orth_err, sigdigits=3), 8)) | Time: $(round(t_factor, digits=3)) s")
        ...
        println("\n  [Elapsed since benchmark start: $(round(time() - run_start, digits=1)) s]")
    ...
    println("\n[Total benchmark wall time: $(round(time() - run_start, digits=1)) s]")

    # ==========================================================================
    # --- Timing Summary Table ---
    # ==========================================================================
    println("\n" * "="^70)
    println("Factorization Time Summary (seconds)")
    println("="^70)
    header = rpad("Scenario", 25) * join([lpad(string(n), 10) for n in n_values])
    println(header)
    for name in sort(collect(keys(all_time)))
        row = rpad(name, 25) * join([lpad(isnan(t) ? "FAIL" : string(round(t, digits=3)), 10) for t in all_time[name]])
        println(row)
    end
    ...
    plt_rel = plot(
        title="QR Factorization Rel. Error vs. Size",
        ylabel="-log10(||QR-A|| / ||A||)",
    ...
    output_filename_rel = "qr_recursive_mixed_rel_error.png"
    savefig(plt_rel, output_filename_rel)

    # 2. Plot Orthogonality Error
    plt_orth = plot(
        title="QR Factorization Orthogonality vs. Size",
    ...
    output_filename_orth = "qr_recursive_mixed_orthogonality.png"
    savefig(plt_orth, output_filename_orth)

    # 3. Plot Factorization Time (log-log, matches the timing script's style)
    plt_time = plot(
        title="QR Factorization Time vs. Size",
    ...
    for (name, results) in all_time
        if any(!isnan, results)
            ...
            plot!(plt_time, n_values, results, label=name, lw=2, ls=line_style, marker=marker_style, ms=5)
        end
    end

    output_filename_time = "qr_recursive_mixed_time.png"
    savefig(plt_time, output_filename_time)

    println("\n" * "="^70)
    println("Benchmark Complete!")
    println("Plots saved as: $output_filename_rel")
    println("                $output_filename_orth")
    println("                $output_filename_time")
    println("="^70)
end
