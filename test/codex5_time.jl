...
# Include the file where you saved the lu_nopiv_recursive! implementation above
# include("lu_nopiv_recursive_impl.jl") # <-- Ensure this filename matches your implementation file
...
function run_time_pure_lu(A_fp32::CuMatrix{Float32}, block_size::Int=4096)
    ...
    # Hits lu_nopiv_recursive!(A::AbstractMatrix, block_size)
    op       = () -> lu_nopiv_recursive!(A_work, block_size)
...
function run_time_mixed_lu(A_fp32::CuMatrix{Float32}, precisions::Vector{DataType}, block_size::Int=4096)
    backend = KernelAbstractions.get_backend(A_fp32)
    local A_mixed_input
    
    # Construct FullMixedPrec hierarchy during the reset phase so structure 
    # allocation and quantization overhead is excluded from the actual kernel timing
    reset_op = () -> begin
        A_mixed_input = FullMixedPrec(copy(A_fp32); precisions=precisions)
        KernelAbstractions.synchronize(backend)
    end
    
    # Hits lu_nopiv_recursive!(A::FullMixedPrec, block_size)
    op = () -> lu_nopiv_recursive!(A_mixed_input, block_size)
...
function check_lu_time_hierarchical()
    ...
    println("="^60)
    println("Starting lu_nopiv_recursive! Performance Benchmark (2k - 16k)")
    println("="^60)
    ...
    for n in n_values
        ...
        for (name, prec_list) in mixed_scenarios
            runtime_ms = run_time_mixed_lu(A_fp32, prec_list, 4096)
    ...
    # ==========================================================================
    plt = plot(
        title="lu_nopiv_recursive! Performance vs. Matrix Size",
    ...
    savefig(plt, "lu_nopiv_recursive_runtimes.png")
    println("\nPlot saved as lu_nopiv_recursive_runtimes.png")
end
