...
@testset "slasd7! test random input of even sizes" begin
    for T in [Float32, Float64]
		...
		#idxq[1:nl] = reverse(Vector(1:nl))
                ...
                NextLA.lasd7_cursor!(icompq, nl, nr, sqre, k, D,
                ...
                @test (givptr_native[] == givptr[])
                @test (givcol_native == givcol)
                @test isapprox(givnum_native, givnum)
                @test isapprox(c_native[], c[])
                @test isapprox(s_native[], s[])
                @test isapprox(info_native[], info[])
