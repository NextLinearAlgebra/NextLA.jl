...
# include("lu_fullmixed_scale_folding.jl") # <-- Ensure this filename matches your implementation file
...
function run_time_pure_lu(A_fp32::CuMatrix{Float32}, block_size::Int=4096)
    ...
    # Hits getrf_recursive!(A::AbstractMatrix, block_size), routing to dispatch_getrf! at leaf
...
function run_time_mixed_lu(A_fp32::CuMatrix{Float32}, precisions::Vector{DataType})
    ...
    # FP16 quantization (_fp16_quantize_block), stochastic rounding, and structure allocation are excluded from LU kernel timing
    reset_op = () -> begin
        A_mixed_input = FullMixedPrec(copy(A_fp32); precisions=precisions)
        KernelAbstractions.synchronize(backend)
    end
    
    # Hits getrf_recursive!(A::FullMixedPrec), utilizing TriMixedPrec extraction and scalar scale folding
...
function check_lu_time_hierarchical()
    ...
    println("="^60)
    println("Starting Scale-Folding getrf_recursive! Benchmark (2k - 16k)")
    println("="^60)
    ...
    # ==========================================================================
    plt = plot(
        title="Scale-Folding Hierarchical LU Performance vs. Matrix Size",
    ...
    savefig(plt, "getrf_recursive_scale_folding_runtimes.png")
    println("\nPlot saved as getrf_recursive_scale_folding_runtimes.png")
end
