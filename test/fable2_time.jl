...
# include("lu_fullmixed_prod_scale.jl") # <-- Ensure this filename matches your implementation file
...
function run_time_pure_lu(A_fp32::CuMatrix{Float32}, block_size::Int=4096)
    ...
    # Hits getrf_recursive!(A::AbstractMatrix, block_size), routing to _getrf_dispatch! at leaf
...
function run_time_mixed_lu(A_fp32::CuMatrix{Float32}, precisions::Vector{DataType})
    ...
    # Hits getrf_recursive!(A::FullMixedPrec), utilizing direct property routing and prod_scale Schur updates
...
function check_lu_time_hierarchical()
    ...
    println("="^60)
    println("Starting Scaled Schur getrf_recursive! Benchmark (2k - 16k)")
    println("="^60)
    ...
    # ==========================================================================
    plt = plot(
        title="Hierarchical LU Performance vs. Matrix Size",
    ...
    savefig(plt, "getrf_recursive_prod_scale_runtimes.png")
    println("\nPlot saved as getrf_recursive_prod_scale_runtimes.png")
end
