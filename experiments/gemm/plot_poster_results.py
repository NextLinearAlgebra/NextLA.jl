#!/usr/bin/env python3
"""Generate poster-ready GEMM figures (one plot per figure) from the v2 result CSVs."""
...
DEFAULT_RESULTS = ROOT / "experiments" / "results" / "gemm" / "v2"
DEFAULT_KBLAS = (
    ROOT / "experiments" / "results" / "gemm"
    / "kblas_tlr_tlr_fp32_fixed_rank_b16_b8.csv"
)
DEFAULT_OUTPUT = ROOT / "experiments" / "figures" / "gemm" / "poster_v2_refined"
...
RANK_STYLE = {1.0 / 16.0: (BLUE, "r/b=1/16"), 1.0 / 8.0: (ORANGE, "r/b=1/8")}
...
def plot_matched_comparison(
    ...
    for divisor in (8, 16):
        marker, linestyle, divisor_label = DIVISOR_STYLE[divisor]
        for rank_ratio in (1.0 / 16.0, 1.0 / 8.0):
            color, rank_label = RANK_STYLE[rank_ratio]
            series = sorted(
                (
                    row for row in matched_rows
                    if integer(row, "q") == divisor
                    and math.isclose(
                        num(row, "rank_over_b"), rank_ratio,
                        rel_tol=0, abs_tol=1e-12,
                    )
                ),
                key=lambda row: integer(row, "N"),
            ...
            ax.plot(
                [integer(row, "N") for row in series],
                [num(row, "kblas_time_over_nextla") for row in series],
                color=color, marker=marker, linestyle=linestyle,
                label=f"{divisor_label}, {rank_label}",
                markeredgecolor="white", markeredgewidth=1.2, zorder=3,
            )
    ax.axhline(1.0, color=AXIS_COLOR, linestyle=":", linewidth=1.3, zorder=1)

    matched_values = [num(row, "kblas_time_over_nextla") for row in matched_rows]
    y_min = min(0.62, min(matched_values) - 0.05)
    y_max = max(1.60, max(matched_values) + 0.08)
    setup_size_axis(ax)
    finish_axes(ax)
    ax.set_ylim(y_min, y_max)
    ax.set_xlabel("N")
    ax.set_ylabel("KBLAS / NextLA time (>1 = NextLA faster)")
    ax.legend(frameon=False, loc="upper right")
    fig.tight_layout()
    paths = save_figure(fig, output, stem, formats, dpi)

    metrics = matched_comparison_metrics(matched_rows)
    return paths, metrics


def matched_comparison_metrics(
    matched_rows: list[dict[str, str]],
) -> dict[str, float | int]:
    matched_values = [num(row, "kblas_time_over_nextla") for row in matched_rows]
    overall_wins = sum(value > 1.0 for value in matched_values)
    large = [
        value for row, value in zip(matched_rows, matched_values)
        if integer(row, "N") >= 8192
    ]
    large_wins = sum(value > 1.0 for value in large)
    large_geomean = geometric_mean(large)
    metrics = {
        "matched_overall_wins": overall_wins,
        "matched_overall_cases": len(matched_values),
        "matched_overall_geomean": geometric_mean(matched_values),
        "matched_large_wins": large_wins,
        "matched_large_cases": len(large),
        "matched_large_geomean": large_geomean,
        "matched_memory_violations": sum(
            num(row, "nextla_memory_ratio") > num(row, "kblas_memory_ratio") * (1.0 + 1e-9)
...
def write_highlights(
    ...
    fastest: dict[str, float | int] | None,
    sources: Sequence[Path],
) -> Path:
    destination = output / "poster_highlights.md"
    lines = [
        ...
        (
            f"- Controlled constant-rank comparison at equal or lower total memory: "
            f"{kblas['matched_overall_wins']}/{kblas['matched_overall_cases']} wins, "
            f"{kblas['matched_overall_geomean']:.2f}× geometric mean."
        ),
        (
            f"- For N≥8192: {kblas['matched_large_wins']}/{kblas['matched_large_cases']} wins, "
            f"{kblas['matched_large_geomean']:.2f}× geometric mean."
        ),
        f"- Memory-matched constraint violations: {kblas['matched_memory_violations']}.",
    ]
    if fastest is not None:
        lines += [
            ...
            "",
            (
                f"- Fastest-workspace NextLA wins {fastest['wins']}/{fastest['cases']} "
                f"with a {fastest['geomean']:.2f}× geometric mean, but uses more memory "
                f"than KBLAS in {fastest['memory_violations']}/{fastest['cases']} cases."
            ),
        ]
    lines += ["", "## Sources", ""]
    lines.extend(
...
def main() -> None:
    ...
    memory_path = results / "memory_v2.csv"
    skewed_path = results / "skewed_data__winners.csv"
    matched_path = results / "kblas_comparison_v2__memory_matched_vs_kblas.csv"
    fastest_path = results / "kblas_comparison_v2__winners__vs_kblas.csv"
    kblas_path = args.kblas.resolve()
    sources = [memory_path, skewed_path, kblas_path, matched_path, fastest_path]
    for path in sources:
        if not path.is_file():
            raise FileNotFoundError(path)

    output = unused_directory(args.output_dir)
    ...
    paths += plot_memory_panel(
        memory_rows, 8, "1a_memory_pareto_b_n8", output, formats, args.dpi
    )
    paths += plot_memory_panel(
        memory_rows, 16, "1b_memory_pareto_b_n16", output, formats, args.dpi
    )
    ...
    paths += plot_precision_grid(
        ...
        "2_precision_speedup_b_n8", output, formats, args.dpi
    )
    ...
    padded_paths, padded_metrics = plot_padded_comparison(
        ...
        "3_nextla_vs_kblas_padded_b_n16", output, formats, args.dpi
    )
    paths += padded_paths
    ...
    paths += plot_constant_rank_comparison(
        fastest_rows,
        "4_constant_rank_best_nextla_b_n16_r_b16",
        output, formats, args.dpi,
    )
    matched_metrics = matched_comparison_metrics(matched_rows)
    kblas_metric_values = {**padded_metrics, **matched_metrics}
