...
double minimum_time_ms(Operation operation, Reset reset, cudaStream_t stream,
                       int warmup, int reps)
{
...
int run(const Config &c)
{
    ...
    if (lll) {
        ...
        const double tlr_ms = minimum_time_ms(operation, reset, stream,
                                              c.warmup, c.reps);
        ...
        const double dense_ms = minimum_time_ms(dense_operation, dense_reset, stream,
                                                c.warmup, c.reps);
        ...
        std::printf("lll,%s,%d,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                    Backend<T>::name(), c.m, c.k, c.n, c.b, c.rank_A, c.rank_B,
                    c.rank_C, final_rank, dense_ms, tlr_ms, dense_ms / tlr_ms,
        ...
        std::printf("lld,%s,%d,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                    Backend<T>::name(), c.m, c.k, c.n, c.b, c.rank_A, c.rank_B,
                    0, 0, dense_ms, tlr_ms, dense_ms / tlr_ms,
                    dense_flops / (dense_ms * 1.0e6), tlr_flops / (tlr_ms * 1.0e6));
        cudaFree(dense_A); cudaFree(dense_B); cudaFree(dense_C);
    }
