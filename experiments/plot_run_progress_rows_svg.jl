# plot_run_progress_rows_svg.jl -- schedule_uniform.svg-style figure, but
# showing ONE DP schedule at two consecutive points in execution order
# (after run i, after run i+1) instead of comparing greedy vs DP.
#
#   julia --project=experiments experiments/plot_run_progress_rows_svg.jl
#
# Writes experiments/figures/run_progress_rows_<name>.svg.
#
# Deliberately reuses full-width row runs only (no column blocking, unlike
# plot_run_progress_svg.jl) -- that is what makes the schedule_uniform.svg
# bracket-and-label-to-the-right treatment work cleanly, since every run then
# spans the same width and "to the right" is a single shared edge. Every
# rectangle is a real `RaggedRowRun` from `_compressed_ftlr_row_runs_dp`.
...
function figure(name, rawA, rawB, bm; budget_frac=0.5, call_cost=0.0)
    ...
    side = 250.0
    xA = 128.0
    gapAB = 210.0   # wider than plot_schedule_svg.jl's -- room for full-length brackets
    xB = xA + side + gapAB
    y0 = 132.0
    W = xB + side + 130
    H = y0 + side + 80
    ...
    push!(out, "</svg>")
    path = joinpath(dir, "run_progress_rows_$(name).svg")
...
# Start from reproducible seeded ranks and use q8 execution-rank bucketing, as
# in schedule_uniform.svg. This is a deliberately legible rank profile: groups
