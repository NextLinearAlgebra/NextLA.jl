...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for n in [256, 1024, 4096]
        A_cpu = randn(Float64, n, n); A_cpu .+= n .* I(n)
        A_gpu = CuMatrix{Float64}(A_cpu)
        n == 256 && getrf_recursive!(copy(A_gpu), 256)   # JIT warm-up

        t = @elapsed (getrf_recursive!(A_gpu, 256); CUDA.synchronize())

        F   = Array(A_gpu)
        L   = tril(F, -1) + Matrix{Float64}(I, n, n)
        U   = triu(F)
        res = norm(L * U - A_cpu) / norm(A_cpu)
        @printf "n=%4d | t=%6.3f s | ‖LU−A‖/‖A‖ = %.3e\n" n t res
    end
    println()
    ...
    precisions = [Float16, Float32, Float32]   # outermost panels → Float16
    A_mp       = FullMixedPrec(CuMatrix{Float32}(A_cpu); precisions=precisions)

    getrf_recursive!(A_mp)

    F_mp  = Array(reconstruct_matrix(A_mp))
    L_mp  = tril(F_mp, -1) + Matrix{Float32}(I, n, n)
    U_mp  = triu(F_mp)
    res   = norm(L_mp * U_mp - A_cpu) / norm(A_cpu)
    @printf "n=%4d | precisions=%s | ‖LU−A‖/‖A‖ = %.3e\n" n string(precisions) res
end