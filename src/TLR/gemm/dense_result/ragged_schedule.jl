"""Exact numerical workspace requirements for a BCLR FoldRight row run."""
...
"""Host-only rank reductions and O(1) range-query metadata for one BCLR GEMM."""
struct BCLRRankPlan
...
@inline _bclr_right_valid(A, B) =
    bclr_outer_order(A) isa TileRowMajor && bclr_outer_order(B) isa TileRowMajor
@inline _bclr_left_valid(A, B) =
    bclr_outer_order(B) isa TileRowMajor && bclr_inner_order(B) isa TileColMajor

@inline function _bclr_prefix(values::Vector{Int})
...
@inline _bclr_range_total(prefix::Vector{Int}, rows::UnitRange{Int}) =
    prefix[last(rows) + 1] - prefix[first(rows)]

function _bclr_rank_plan(A, B)
    ...
    @inbounds for j in 1:qn, k in 1:qk
        b_col_k_prefix[j, k + 1] = b_col_k_prefix[j, k] + _bclr_rank(B, k, j)
    end
    ...
    profile = RaggedWorkspaceProfile(
        ...
        right === nothing ? nothing : _bclr_prefix(right),
        left === nothing ? nothing : _bclr_prefix(left),
        right_flops === nothing ? nothing : _bclr_prefix(right_flops),
        left_flops === nothing ? nothing : _bclr_prefix(left_flops),
        isempty(row_bytes) ? 0 : maximum(row_bytes), maximum_bytes,
    )
    return BCLRRankPlan(a_k_prefix, b_row_ranks, b_col_ranks, b_col_k_prefix,
                        b_col_prefix, b_first_active_col, pair_ranks, b_total_rank,
                        profile)
end

_bclr_workspace_profile(A, B) = _bclr_rank_plan(A, B).profile

function gemm_minimum_workspace_bytes(A::BCLRMatrix, B::BCLRMatrix;
                                      transA::Char='N', transB::Char='N')
    return _bclr_rank_plan(logical_operand(A, transA), logical_operand(B, transB)).profile.minimum
end

function gemm_maximum_workspace_bytes(A::BCLRMatrix, B::BCLRMatrix;
                                      transA::Char='N', transB::Char='N')
    return _bclr_rank_plan(logical_operand(A, transA), logical_operand(B, transB)).profile.maximum
end

"""Contiguous output-row runs greedily packed under an exact ragged budget."""
function _bclr_row_runs(profile::RaggedWorkspaceProfile, budget::Int)
    ...
    while i <= length(profile.row_bytes)
        j = i - 1
        while j < length(profile.row_bytes) && _bclr_select_fold(profile, i:(j + 1), budget) !== nothing
            j += 1
        end
        # A zero-work row is allowed even for a zero byte budget.
        j >= i || (j = i)
        fold = _bclr_select_fold(profile, i:j, budget)
        fold === nothing && throw(ArgumentError("workspace cannot schedule BCLR row $i"))
...
function _bclr_select_fold(profile::RaggedWorkspaceProfile, rows::UnitRange{Int}, budget::Int)
    right_bytes = profile.right_byte_prefix === nothing ? typemax(Int) :
        _bclr_range_total(profile.right_byte_prefix, rows)
    left_bytes = profile.left_byte_prefix === nothing ? typemax(Int) :
        _bclr_range_total(profile.left_byte_prefix, rows)
    ...
    right_flops = _bclr_range_total(profile.right_flop_prefix, rows)
    left_flops = _bclr_range_total(profile.left_flop_prefix, rows)
    return right_flops <= left_flops ? :right : :left
end

function _prepare_bclr_workspace(A, workspace, profile::RaggedWorkspaceProfile)
    T = eltype(A)
    ws = if workspace isa Integer
        ...
        DenseGemmWorkspace(_bclr_parent(A), bytes)
