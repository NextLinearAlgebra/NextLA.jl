...
@kernel function _fused_truncate_kernel!(
    ...
    if j == 1
        @inbounds for _ in 1:(R - 1), i in 1:(R - 1)
            ni  = norms_s[i]; ni1 = norms_s[i + 1]
            if ni < ni1
                norms_s[i] = ni1; norms_s[i + 1] = ni
                pi = perm_s[i];   perm_s[i] = perm_s[i + 1]; perm_s[i + 1] = pi
            end
        end
        budget = eps_sq; k_val = R
    ...
    bU  = size(U, 1); bV = size(V, 1)
    if j <= k
        @inbounds for row in 1:bU; U_out[row, j, ob] = U[row, src, ob]; end
        @inbounds for row in 1:bV; V_out[row, j, ob] = V[row, src, ob]; end
    else
        @inbounds for row in 1:bU; U_out[row, j, ob] = zero(T); end
        @inbounds for row in 1:bV; V_out[row, j, ob] = zero(T); end
    end
end

# ─── Tile-view helpers ────────────────────────────────────────────────────────

function _tile_views(A::AbstractMatrix, layout::TileMap, obs::Vector{Int})
...
function alloc_workspace(A_tlr::TLRMatrix{<:Any,T}) where {T}
    ...
    b         = layout.tile_m
    mt, nt    = size(layout)
    tail_m    = max(layout.m - (mt - 1) * b, 1)
    tail_n    = max(layout.n - (nt - 1) * b, 1)
...
function _compress_category!(
    ...
    A_tiles     = _tile_views(A, layout, cat.obs)
...
function _compress_categories!(
    ...
    if A_tlr.backend isa KernelAbstractions.CPU
        for cat in cats
            _compress_category!(A_tlr.backend, A_tlr.layout, A, cat, eps_sq, alg, ns_iters)
        end
    else
...
function compress!(A_tlr::TLRMatrix{<:Any,T}, A::AbstractMatrix{T},
    ...
    size(A, 1) == A_tlr.layout.m && size(A, 2) == A_tlr.layout.n ||
        throw(DimensionMismatch("A dimensions must match A_tlr"))
    A_tlr.layout.m == A_tlr.layout.n && A_tlr.layout.tile_m == A_tlr.layout.tile_n ||
    ...
    # Diagonal tiles
    n_diag = ndiag_tiles(A_tlr.layout)
    if n_diag > 0
        _copy_diag_kernel!(A_tlr.backend)(
            A_tlr.D, A, A_tlr.layout;
            ndrange = (A_tlr.layout.tile_m, A_tlr.layout.tile_n, n_diag),
        )
    end
