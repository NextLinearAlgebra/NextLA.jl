...
function _tlr_dense_gemm!(C, A::LogicalTLROperand{<:Any,<:TLRMatrix{<:Any,T}},
                          B::LogicalDenseOperand, alpha, beta, budget::Int, compute) where {T}
    _scale_output!(C, beta)
    r = maxrank(A)
    (isempty(C) || r == 0) && return C
    mt, kt = tilegrid_size(A)
    n = size(B, 2)
    width = clamp(div(budget, max(r * sizeof(T), 1)), 1, n)
    work = allocate(get_backend(A), T, r, width)

    @inbounds for i in 1:mt, cols in Iterators.partition(1:n, width)
...
function _dense_tlr_gemm!(C, A::LogicalDenseOperand,
                          ...
                          alpha, beta, budget::Int, compute) where {T}
    _scale_output!(C, beta)
    r = maxrank(B)
    (isempty(C) || r == 0) && return C
    ...
    work = allocate(get_backend(B), T, height, r)
