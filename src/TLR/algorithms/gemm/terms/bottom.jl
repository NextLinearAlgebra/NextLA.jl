...
No-op when `m_A % b == 0`.
"""
function tlr_gemm_bpanel_by_int(C, A::LogicalTLROperand{<:Any,<:TLRDenseDiagMatrix}, B::LogicalTLROperand{<:Any,<:TLRDenseDiagMatrix}, alpha::T; beta::T=one(T), budget::Int) where {T}
    ...
    gemm_batched!('T', _opchar(B), one(T),
    ...
    gemm_batched!('N', 'N', alpha,
        ...
        beta, [_output_tile_view(C, A, B, mt, j) for j in 1:qkA])
    ...
    @inbounds for jrange in Iterators.partition(1:qkA, maxJ)
        ...
        for kpos in 1:nk_off_per_col
            gemm_batched!('N', 'N', alpha,
                ...
                one(T),
                [_output_tile_view(C, A, B, mt, j) for j in jrange])
        end
    end
    return C
end
...
"""
function tlr_gemm_corner_by_bpanel(C, A::LogicalTLROperand{<:Any,<:TLRDenseDiagMatrix}, B::LogicalTLROperand{<:Any,<:TLRDenseDiagMatrix}, alpha::T; beta::T=one(T)) where {T}
    ...
    gemm_batched!(_dense_op(corner), 'N', one(T),
                  physical(A).D_corner, BU, zero(T), Swork)
...
function tlr_gemm_bpanel_by_int(C, A::LogicalTLROperand{<:Any,<:TLRMatrix}, B::LogicalTLROperand{<:Any,<:TLRMatrix}, alpha::T;
...
function tlr_gemm_corner_by_bpanel(C, A::LogicalTLROperand{<:Any,<:TLRMatrix}, B::LogicalTLROperand{<:Any,<:TLRMatrix}, alpha::T;
